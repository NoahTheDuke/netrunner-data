...
const packDir = resolve(import.meta.dirname, '..', 'pack');
fs.readdirSync(packDir).forEach(file => {
  if (file.endsWith('.json')) {
    ...
    if (needsUpdates) {
      fs.writeFile(resolve(import.meta.dirname, '..', 'pack', file), JSON.stringify(json), err => {
        if (err) {
          console.error(err);
        }
      });
...
const cardsDir = resolve(import.meta.dirname, '..', 'v2/cards');
...
const printingsDir = resolve(import.meta.dirname, '..', 'v2/printings');
